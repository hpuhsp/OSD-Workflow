...
function commandSummary(result, fallback) { return String(result.status === 0 ? result.stdout || fallback : result.stderr || result.stdout || fallback).trim().slice(0, 2000); }
...
function runVerificationCheck({ id, label, command, required, target, spawn, executed }) {
  ...
  if (executed.has(command)) {
    ...
    return { id, label, required, command, status: previous.exit_code === 0 ? "passed" : "failed", exit_code: previous.exit_code, summary: previous.summary, reused_from: previous.id };
  }
  ...
  const check = { id, label, required, command, status: exitCode === 0 ? "passed" : "failed", exit_code: exitCode, summary: commandSummary({ ...result, status: exitCode }, `${label} ${exitCode === 0 ? "passed" : "failed"}`) };
...
export function verifyDelivery(options, { output = console.log, spawn = nodeSpawnSync } = {}) {
  ...
  const evidence = { schema: "osd-verification-evidence/v2", feature: state.feature, mode: state.mode, strategy: state.strategy, command, exit_code: exitCode, checks, observed_at: now(), summary: checks.map((check) => `${check.label}: ${check.status}`).join("; "), specification: state.specification_path };
...
export function doctorProject(options, { output = console.log, env = process.env, spawn = nodeSpawnSync } = {}) {
  ...
  output(`OSD doctor: ${target}`); output(`  config: ${configCheck.ok ? "ok" : "problem"} (${configCheck.detail})`); output(`  OpenSpec: ${capabilities.openspec.available && capabilities.openspec.workspace ? "available" : "unavailable"} (${capabilities.openspec.detail}; workspace ${capabilities.openspec.workspace ? "present" : "missing"})`); output(`  Superpowers: ${capabilities.superpowers.available ? "available" : "unavailable"} (${capabilities.superpowers.detail})`); output(`  verification command: ${capabilities.verificationCommand.detail}`); output(`  unit test command: ${capabilities.unitTestCommand.detail}`); for (const agent of agents) output(`  Agent rule ${agent.agent}: ${agent.present ? "present" : "missing"}`); for (const adapter of adapters) output(`  ${adapter.stage}: ${adapter.selected || "unresolved"} (${adapter.status}${adapter.fallbackUsed ? ", fallback used" : ""})`); return report;
}
